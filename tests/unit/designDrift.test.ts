 ...
 *     уже замерло;
 *
 *   • цвет. Литерал в стиле не следует ни за темой, ни за глубиной, ни за
 *     акцентом. Значки площадок в окне импорта были набраны фирменными цветами
 *     сервисов — и на светлых темах оказывались единственными кричащими пятнами
 *     в окне. Их починили, а предохранителя не поставили, и тем же способом
 *     мимо темы успели уехать затемнение квадратного мини-плеера и стоп
 *     градиента в «Потоке».
 */
...
describe('оформление в обход темы', () => {
  ...
  it('цвет берётся из темы, а не пишется литералом', () => {
    /*
     * Ловится не любой шестнадцатеричный код в файле, а только тот, что стоит
     * значением свойства оформления: `color`, `background`, `border` и
     * соседи. Иначе под правило попали бы подсказка «запись вида #8fc7ff» в
     * настройках акцента и пояснения в комментариях — то есть текст, а не вид.
     *
     * Исключений у правила нет намеренно. Прозрачность пишется словом
     * `transparent`, а всё остальное — токеном: место для литералов ровно
     * одно, `theme.css`, и оно вне досягаемости этой проверки.
     */
    const literalColour =
      /(?:color|background|backgroundColor|borderColor|border|boxShadow|fill|stroke|outline|backgroundImage|textShadow)\s*:\s*['"`][^'"`]*(?:#[0-9a-fA-F]{3,8}|rgba?\()/;

    expect(
      offenders(literalColour),
      'цвет мимо темы: он не следует ни за пресетом, ни за глубиной, ни за акцентом'
    ).toEqual([]);
  });
