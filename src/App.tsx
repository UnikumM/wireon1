...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    /*
     * Просим систему не вычищать хранилище страницы — на запуске, а не только
     * при первом сохранении в офлайн: тот, кто ни разу ничего не скачивал, не
     * просил об этом ни разу.
     *
     * Оговорка, замеренная на устройстве: WebView Android отвечает на эту
     * просьбу отказом — `persist()` возвращает `false` и `persisted()` остаётся
     * `false`. То есть на телефоне строка ничего не защищает, и рассчитывать на
     * неё нельзя; сохранённые треки бережёт не она, а переезд на файлы
     * (`offlineFiles.ts`). Смысл она имеет в сборке для браузера, где просьбу
     * удовлетворяют, и стоит копейки — поэтому остаётся.
     */
