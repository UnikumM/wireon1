...
/**
 * Ниже этого совпадение названий считается случайным.
 *
 * Треть общих слов — это уже «Test» против «Test YouTube Track»: одно слово из
 * трёх. Такие пары встречаются у одного исполнителя постоянно, и без отдельного
 * штрафа они выигрывали за счёт совпавших исполнителя и длительности.
 */
export const TITLE_MATCH_FLOOR = 0.34;
...
export function scoreCandidate(target: MatchTarget, candidate: UnifiedTrack): MatchResult<UnifiedTrack> {
  ...
  // Совсем чужое название не вытягивается ничем остальным.
  //
  // Так подменялась песня при воспроизведении: у кандидата с обрезанным
  // названием («Test» вместо «Test YouTube Track») совпадали исполнитель и
  // длительность, и суммы хватало с запасом — хотя это заведомо другая запись
  // того же артиста. Название — единственное, что вообще отличает песни внутри
  // одного исполнителя, поэтому его слабость должна стоить дорого.
  if (titleRatio < TITLE_MATCH_FLOOR) {
    score -= 40;
    notes.push('название почти не совпадает');
  }
