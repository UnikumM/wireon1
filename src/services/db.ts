...
export interface MatchLinkRecord {
  ...
  manual?: boolean;
  /**
   * Идентификатор строки у чужого каталога, если он был известен.
   *
   * Ключом не служит — ключ собран из названия, чтобы работать и там, где
   * идентификатора нет вовсе. Лежит рядом, чтобы по связи можно было понять,
   * откуда она взялась, и чтобы будущий точный поиск было на что опереть.
   */
  foreignId?: string;
