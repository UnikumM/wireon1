...
export const useAuthStore = create<AuthStore>((set, get) => ({
  ...
  restoreSession: async () => {
    ...
    try {
      ...
      /*
       * Срок сессии сдвигается вперёд при каждом удачном входе в приложение.
       *
       * До этого он ставился один раз при входе и был ровно неделей: человек,
       * открывающий приложение каждый день, всё равно оказывался выкинут на
       * восьмой — при том что Discord его токен в этот самый момент принял, о
       * чём и говорит `validateSession` строкой выше. Выкидывали мы, а
       * выглядело это как «кикает из аккаунта Discord».
       *
       * Неделя как срок остаётся: заброшенная сессия должна протухать. Но
       * считается она от последнего запуска, а не от давнего входа.
       */
      saveStoredSession(validated, token);
